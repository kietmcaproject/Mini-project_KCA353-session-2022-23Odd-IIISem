  ...
  <ul class="d-flex align-items-center">
    ...
    <li class="nav-item dropdown pe-3">
      ...
      <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
        <img src="assets/img/lgin.png" alt="Profile" class="rounded-circle">
        <span class="d-none d-md-block dropdown-toggle ps-2">Nishant sharma</span>
    ...
    <tr>
      ...
      if( $nor==0 ){
        ...
        while($row = mysqli_fetch_assoc($result)){
          echo "<tr><td>{$n}</td><td>{$row['company_name']}</td><td>{$row['eligible_branches']}</td><td>{$row['job_profile']}</td><td>{$row['ctc']} LPA</td><td><input type='button' value='Apply' class='btn btn-primary' onClick='window.open('{$row['link']}');'></td></tr>\n";
            $n++;
        }
      }
    ?>
